...
const deploy = async ({ key, value, image: img, namespace }) => {
  log(`Deploying ${image}:${version} on Kubernertes`, { key, value, img, namespace });
  const { status } = await spawnSync('npx', ['@endeavorb2b/rancher2cli', 'dl', key, value, img, namespace]);
